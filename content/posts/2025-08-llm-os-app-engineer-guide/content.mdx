...
理解長上下文模型（如 Gemini 1.5, Claude Sonnet 4）如何從根本上改變 RAG 的設計假設，並闡明為何「拋棄檢索，只靠長上下文」並非萬能解方，從而引出「分層式混合架構」的必要性。
...
- **成本考量 (Cost)：** 以 Gemini 1.5 Pro 為例，處理 1M token 的輸入成本可能高達 $7。如果每次查詢都如此昂貴，系統將不具備商業可行性。

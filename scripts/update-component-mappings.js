...
const COMPONENT_LOADER_PATH = path.join(
  process.cwd(),
...
async function updateComponentMappings() {
  ...
  const mappings = postsWithComponents
    .map((post) => {
  ...
  const updatedContent = currentContent.replace(
    /(const componentMappings: Record<string, \(\) => Promise<unknown>> = {)([\s\S]*?)(};)/,
    `$1\n${mappings}\n$3`
  );
  ...
  console.log(
    `✅ Updated component mappings for ${postsWithComponents.length} posts`
  );
